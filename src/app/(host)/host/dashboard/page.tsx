  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-stone-500 text-sm mt-0.5">Overview of all court events and player check-ins</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white border border-stone-200/60 rounded-2xl p-5">
          <h2 className="text-xs font-bold text-stone-500 uppercase tracking-wider mb-4">Most Popular Sessions</h2>
          <div className="space-y-3">
            {topEvents.map((e, i) => (
              <div key={e.id} className="flex items-center gap-3">
                <span className="w-6 h-6 rounded-lg bg-stone-100 flex items-center justify-center text-[10px] font-bold text-stone-500 tabular-nums">
                  {i + 1}
                </span>
                ...
                <span className="text-sm font-bold text-stone-700 tabular-nums">{e._count.attendances}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Recent Attendances */}
        <div className="bg-white border border-stone-200/60 rounded-2xl p-5">
          <h2 className="text-xs font-bold text-stone-500 uppercase tracking-wider mb-4">Recent Check-ins</h2>
            ...
            {recentAttendances.map((a) => (
              <div key={a.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[11px] text-stone-400 truncate">{a.event.title.split(" - ")[0]}</p>
                </div>
      ...
      {/* RSVP vs Attendance */}
      <div className="bg-white border border-stone-200/60 rounded-2xl p-5">
        <h2 className="text-xs font-bold text-stone-500 uppercase tracking-wider mb-1">RSVP vs Attendance</h2>
        <p className="text-[11px] text-stone-400 mb-4">Upcoming & today's sessions — how many who RSVP'd actually showed up</p>
        {upcomingWithRsvps.length === 0 ? (
          <p className="text-sm text-stone-400 text-center py-6">No upcoming sessions.</p>
        ) : (
          <div className="space-y-2.5">
                ...
                <div key={e.id} className="flex items-center gap-3 py-1.5">
                  ...
                  <div className="flex items-center gap-4 flex-shrink-0">
                    <div className="text-center">
                      <p className="text-sm font-bold text-purple-600 tabular-nums">{rsvps}</p>
                      <p className="text-[10px] text-stone-400">RSVPs</p>
                    </div>
                    <div className="text-center">
                      <p className="text-sm font-bold text-emerald-600 tabular-nums">{attended}</p>
                      <p className="text-[10px] text-stone-400">Showed</p>
                    </div>
                    {isPast && showRate && (
                      <div className="text-center">
                        <p className={`text-sm font-bold tabular-nums ${rate >= 70 ? "text-green-600" : rate >= 40 ? "text-amber-600" : "text-red-500"}`}>
                          {rate}%
                        </p>
